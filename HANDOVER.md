...
Localhost :8795, token-gated (token in `~/.config/powpowpow/site.env`). Public pow.moltwork.com NOT exposed — tunnel missing, see BLOCKERS.md #1. Black/white/grey only, chunky borders, monospace numbers, no opinions. Style guide in site/STYLE.md. Left rail for HOME + BTC + coins + CHAT. Each coin: subtabs Overview/Epoch/Mining/Flow/Signals. BTC tab: Overview (security spend, EH/s, difficulty, height, fees), Epoch (halving countdown + subsidy schedule), Flow (fee market — no venue listing), Mining (security/hashrate/difficulty charts + garden-computed hashprice), Signals (baseline role: XMR/QUBIC hooks). Live ticker streams `/api/ticks` (SSE, 2s events) with `/api/live` polling fallback. XMR Mining tab shows security spend, $/MH/day hashprice, hardware profitability, security-spend chart. Chat through Pi harness with garden context. Bottom panel auto-refreshes ops every 60s.

## API + MCP (what powops can query)

REST (token-gated): `/api/health /api/live /api/ticks (SSE) /api/signals /api/factors /api/state /api/state_series /api/history /api/cards /api/cards_history /api/chain /api/analysis /api/analytics /api/xmr_full /api/opportunity /api/btc /api/btc_series /api/epoch_series /api/brief /api/page /api/ops`, POST `/api/chat`.
